...
KM_DELTA_NOISE = 0.07# Kinematic car delta noise std dev
...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    curr_steering = (self.last_servo_cmd - self.STEERING_TO_SERVO_OFFSET) / self.STEERING_TO_SERVO_GAIN
    ...
    noisy_speed = np.random.normal(curr_speed, KM_V_NOISE, x.shape)
    noisy_steering = np.random.normal(curr_steering, KM_DELTA_NOISE, theta.shape)  
      # Propagate particles through kinematic model with noisy controls
    
    B = np.arctan(np.tan(noisy_steering) / 2)
    theta_new = theta + noisy_speed / self.CAR_LENGTH * np.sin(2*B) * (msg.header.stamp - self.last_vesc_stamp).to_sec()
    x = x + self.CAR_LENGTH * (np.sin(theta_new) - np.sin(theta)) / np.sin(2*B)
    y = y + self.CAR_LENGTH * (-np.cos(theta_new) + np.cos(theta)) / np.sin(2*B)
    self.particles[:]
      # Sample model noise for each particle
    x = np.random.normal(x, KM_X_FIX_NOISE, x.shape)
    y = np.random.normal(y, KM_Y_FIX_NOISE, y.shape)
    theta_new = np.random.normal(theta_new, KM_THETA_FIX_NOISE, theta_new.shape)
...
TEST_SPEED = 0.5 # meters/sec
TEST_STEERING_ANGLE = 0.8 # radians
TEST_DT = 2 # seconds
