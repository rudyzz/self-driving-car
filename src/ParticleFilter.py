...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    x_permissible, y_permissible = np.where(self.permissible_region==1)
    index = np.random.choice(len(x_permissible), self.N_PARTICLES)
    particles_tmp = np.zeros(self.particles.shape)
    particles_tmp[:,0] = x_permissible[index]
    particles_tmp[:,1] = y_permissible[index]
    particles_tmp[:,2] = 2.0*np.pi*np.random.random(self.N_PARTICLES)
    Utils.map_to_world(particles_tmp, self.map_info)
  ...
  def clicked_pose_cb(self, msg):
    ...
    self.particles[:,0] = msg.pose.pose.position.x + np.random.normal(0.0, 1.0, self.N_PARTICLES)
    self.particles[:,1] = msg.pose.pose.position.y + np.random.normal(0.0, 1.0, self.N_PARTICLES)
    self.particles[:,2] = Utils.quaternion_to_angle(msg.pose.pose.orientation) + np.random.normal(0.0, 0.01, self.N_PARTICLES)
    self.weights[:] = float(1/self.N_PARTICLES)
    self.state_lock.release()
