...
class ReSampler:
  ...
  def resample_naiive(self):
    ...
    #initialize new nx3 particles bins matrics: [x,y,theta] x n
    new_particles = np.zeros((n_particles,3))

    # update the new particles & weights that corresponds to the random number bin_index falls into
    for m in xrange(0,n_particles):
      #draw M random numbers between 0.0 and 1.0
      r = np.random.rand() * float(1)
      print(r)
      c = self.weights[0]
      i = 0
  ...
  def resample_low_variance(self):
    ...
    # YOUR CODE HERE
    # #numbers of the particles rows
    n_particles = self.particles.shape[0]

    # #initialize new nx3 particles matrics: [x,y,theta] x n
    new_particles = np.zeros((n_particles,3))

    #Return random floats in the half-open interval [0.0, 1/M).
    M_inverse = 1.0/float(n_particles)
    r = np.random.uniform(0, M_inverse)
    # print("--------------------------")
    # print("this is r:")
    # print(r)
    c = self.weights[0]
