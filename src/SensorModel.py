...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    # downsampled_ranges = None

    original_ranges = np.copy(msg.ranges).astype(np.float32)
    # MARK invalid data
    original_ranges[original_ranges[:] == 0.0] = self.MAX_RANGE_METERS
    original_ranges[np.isnan(original_ranges)] = self.MAX_RANGE_METERS

    # initialize self.laser_angles
    # if not isinstance(self.laser_angles, np.ndarray):
    #   self.laser_angles = np.linspace(msg.angle_min, msg.angle_max, len(msg.ranges))

    angle_data = np.linspace(msg.angle_min, msg.angle_max, len(msg.ranges))
    
    if (self.EXCLUDE_MAX_RANGE_RAYS):
        # Get rid of invalid data
      filtered_ranges = original_ranges[original_ranges[:] != self.MAX_RANGE_METERS]
      filtered_angles = angle_data[original_ranges != self.MAX_RANGE_METERS]

      original_ranges = filtered_ranges
      angle_data = filtered_angles

      obs = [[], []]

      steps = np.array(original_ranges).shape[0]

      for x in range(steps):
        temp_range = original_ranges[x*self.LASER_RAY_STEP]
        temp_angle = angle_data[x*self.LASER_RAY_STEP]

        obs[0].append(temp_range)
        obs[1].append(temp_angle)
        obs = np.array(obs).astype(np.float32)
        obs = (obs[0], obs[1])

    #   step_size = self.LASER_RAY_STEP*(float(len(filtered_ranges))/float(len(original_ranges)))

    #   index = np.arange(0, len(filtered_ranges), step_size).astype(int)
    #   downsampled_ranges = filtered_ranges[index]
    #   self.downsampled_angles = filtered_angles[index]
    # else:
    #   index = np.arange(0, len(original_ranges), self.LASER_RAY_STEP).astype(int)
    #   downsampled_ranges = original_ranges[index]
    #   self.downsampled_angles = self.laser_angles[index]

    # obs = (downsampled_ranges, np.copy(self.downsampled_angles).astype(np.float32))
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for d in xrange(table_width):
      ...
      for r in xrange(table_width):
        ...
        if r >= 0 and r <= d:
          prob += Z_SHORT * np.exp(-1 * r)

        if r >= 0 and r < max_range_px:
          prob += Z_RAND * float(1.0 / max_range_px)
          prob += Z_HIT * np.exp(-0.5 * (z ** 2.0) / b) / np.sqrt(2.0 * np.pi * b) 
...
if __name__ == '__main__':
  ...
  bag_path = rospy.get_param("~bag_path", '/home/car-user/racecar_ws/src/lab4/bags/laser_scans/laser_scan1.bag')
