...
class LineFollower:
 ...
 car_pos = np.array([cur_pose[0],cur_pose[1]])
 plan_pos = np.array([self.plan[self.index][0],self.plan[self.index][1]])
 offset = T * ((plan_pos-car_pos).reshape(2,1))
 print("car_pose: ",car_pos)
 print("plan_pose: ",plan_pos)
 print(offset)
 if offset[0] > 0:
  break
 else:
  self.index=self.index+1
    ...
    theta=cur_pose[2]
    trans_matrix=np.reshape([cur_pose[0],cur_pose[1]],(2,1))
    rotation_matrix=[[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]]
    invert_rotation=np.reshape(inv(rotation_matrix),(2,2))
    goal_carPose=[]
    ...
    for msg in range(len(self.plan)-1):
      update_curPlan=np.reshape([self.plan[msg][0],self.plan[msg][1]],(2,1))
      goal_carPose.append(np.matmul(invert_rotation,(update_curPlan-trans_matrix)))
      if (goal_carPose[msg][0])<0:
        self.index=msg
        #print("goal_index",self.index)
      if self.index > len(self.plan)-self.plan_lookahead:
       return False, 0.0
    ...
    goal_idx=self.index
    translation_error=self.plan[goal_idx][0]-cur_pose[0]
    ...
    # YOUR CODE HERE
    error = self.translation_weight * translation_error + 10 * rotation_error
  ...
  def compute_steering_angle(self, error):
    ...
    if len(self.error_buff) > 0 :
      ...
      error_diff=error-self.error_buff[-1][0]
      deriv_error=error_diff/dt
    ...
    integ_error= sum(row[0] for row in self.error_buff)*dt
...
def main():
  ...
  pose_topic = rospy.get_param('~pose_topic','/pf/viz/inferred_pose') #'/car/car_pose'# Default val: '/car/pose'
  plan_lookahead = rospy.get_param('~plan_lookahead',5)# Starting val: 5
  translation_weight = rospy.get_param('~translation_weight',1)# Starting val: 1.0
  rotation_weight = rospy.get_param('~rotation_weight',0)# Starting val: 0.0
  kp = rospy.get_param('~kp',8)# Startinig val: 1.0
  ki = rospy.get_param('~ki',10)# Starting val: 0.0
  kd = rospy.get_param('~kd',1)# Starting val: 0.0
  ...
  plan_list=[]
  for msg in plan.poses:
   plan_item=np.array([msg.position.x,
                         msg.position.y,
  ...
  # YOUR CODE HERE
  print("LINE")
  LineFollower(plan_list, pose_topic, plan_lookahead, translation_weight,
               rotation_weight, kp, ki, kd, error_buff_length, speed)
